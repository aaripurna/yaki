defmodule YakiCoreTest do
  ...
  defmodule MyUploader do
    ...
    variants thug_life: {100, 100, resize_to: :fill},
              two_pac: {100, 100}, thumbnail: {200, 300, [resize_to: :limit]}
  ...
  describe "adapter/1" do
    test "it adds one adapter" do
      assert MyUploader.adapters() == [local: MyAdapter]
    end
  end

  describe "variant/2" do
    test "it adds new variant" do
      assert (MyUploader.variants() |> Enum.member?({:thug_life, {100, 100, [resize_to: :fill]}}) == true)
    end
